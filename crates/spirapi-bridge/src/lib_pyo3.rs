...
impl SpiraPiEngine {
    pub fn initialize(spirapi_path: PathBuf) -> Result<(), SpiraChainError> {
        ...
        Python::with_gil(|py| {
            ...
            if !spirapi_src.exists() {
                return Err(SpiraChainError::Internal(
                    format!("SpiraPi src directory not found: {}", spirapi_src.display())
                ));
            }

            path.call_method1("insert", (0, spirapi_src.to_str().unwrap()))
                .map_err(|e| SpiraChainError::Internal(format!("Failed to add to sys.path: {}", e)))?;

            info!("   ✓ Added {} to Python path", spirapi_src.display());

            let math_module = py.import_bound("math_engine.pi_sequences")
                .map_err(|e| SpiraChainError::Internal(format!("Failed to import math_engine: {}", e)))?;
            
            let engine_class = math_module.getattr("PiDIndexationEngine")
                .map_err(|e| SpiraChainError::Internal(format!("Failed to get PiDIndexationEngine: {}", e)))?;
            
            let pi_engine = engine_class.call0()
                .map_err(|e| SpiraChainError::Internal(format!("Failed to create PiDIndexationEngine: {}", e)))?;

            info!("   ✓ PiDIndexationEngine initialized");

            let ai_module = py.import_bound("ai.semantic_indexer")
                .map_err(|e| SpiraChainError::Internal(format!("Failed to import ai.semantic_indexer: {}", e)))?;
            
            let indexer_class = ai_module.getattr("SemanticPiIndexer")
                .map_err(|e| SpiraChainError::Internal(format!("Failed to get SemanticPiIndexer: {}", e)))?;
            
            let db_path = spirapi_path.join("pi_schemas.db").to_str().unwrap().to_string();
            let semantic_indexer = indexer_class.call1((db_path,))
                .map_err(|e| SpiraChainError::Internal(format!("Failed to create SemanticPiIndexer: {}", e)))?;
    ...
    pub fn generate_pi_coordinate_real(
        ...
        let engine = engine_guard.as_ref()
            .ok_or_else(|| SpiraChainError::Internal("Engine not initialized - call initialize_spirapi() first".to_string()))?;
        ...
        Python::with_gil(|py| {
            ...
            let result = engine.pi_engine.call_method(py, "generate_unique_identifier", (), Some(kwargs))
                .map_err(|e| SpiraChainError::Internal(format!("Python method call failed: {}", e)))?;
            ...
            let identifier = result_dict.get_item("identifier")
            ...
            let id_str: String = identifier.extract()
                .map_err(|e| SpiraChainError::Internal(format!("Failed to extract string: {}", e)))?;
    ...
    pub fn semantic_index_content_real(content: &str, content_type: &str) -> Result<SemanticIndexResult, SpiraChainError> {
        ...
        Python::with_gil(|py| {
            ...
            let result_dict = result.cast_as::<PyDict>(py)
                .map_err(|e| SpiraChainError::Internal(format!("Result not dict: {}", e)))?;

            let pi_id = result_dict.get_item("pi_id")
                .map_err(|e| SpiraChainError::Internal(format!("Get pi_id failed: {}", e)))?
                .ok_or_else(|| SpiraChainError::Internal("pi_id missing".to_string()))?
                .extract::<String>()
                .unwrap_or_else(|_| format!("pi_{}", hex::encode(&blake3::hash(content.as_bytes()).as_bytes()[..8])));

            let content_hash_obj = result_dict.get_item("content_hash")
                .unwrap_or(None);
            let content_hash = if let Some(obj) = content_hash_obj {
                obj.extract::<String>().unwrap_or_else(|_| blake3::hash(content.as_bytes()).to_hex().to_string())
            } else {
                ...
                if let Ok(analysis_dict) = analysis.downcast::<PyDict>() {
                    if let Ok(Some(embedding)) = analysis_dict.get_item("embedding") {
                        if let Ok(list) = embedding.downcast::<PyList>() {
                            list.iter().filter_map(|item| item.extract::<f32>().ok()).collect()
                        } else {
                            vec![0.0; 384]
                        }
                    } else {
    ...
    pub fn calculate_pi_real(precision: usize, algorithm: &str) -> Result<String, SpiraChainError> {
        ...
        Python::with_gil(|py| {
            ...
            let value = result_dict.get_item("value")
